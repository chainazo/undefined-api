...
// GET 특정밋업에 참석하는 사람들 /meetups/:meetup_id/attending
Router.route('/:meetup_id/user/attending')
.get((req, res) =>{
  (async () => {
    const attendedMembers = await Reservation.findAll({
      include: [{
        model: User,
        as: 'user',
      }]
...
// GET 참석확정된사람을 알려주는 API  /meetups/:meetup_id/attending
Router.route('/:meetup_id/user')
.get((req, res) =>{
  (async () => {
    const attendedMembers = await Reservation.find({
      where: {
